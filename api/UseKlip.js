...
const isMobile = width >= 1280 ? false : true;
...
export const getAddress = (setQrvalue, callback) => {
  axios
    ...
    .then((response) => {
        ...
        let url;
        // try{
        //     url=getKlipAccessUrl("android", request_key);
        // } catch(err){
        //     url=getKlipAccessUrl("QR", request_key)
        // }
        url=Platform.OS==='android' ?  getKlipAccessUrl("android", request_key) : getKlipAccessUrl("ios", request_key);
        Linking.openURL(url);
        let timerId = setInterval(() => {
            axios
            .get(
...
//지갑 주소 변경하는 것과 다르게 Post에 다른 options을 넣어주어서
//smart contract를 수행해주는 코드임
